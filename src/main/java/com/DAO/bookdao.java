...
public class bookdao {
    ...
    public bookdao(Connection conn) {
     ...
     public boolean AddBook( bookdtl bd) {
        boolean f=false;
        try{
            String sql="insert into book(BookName,BookCategiries, SubCategiries,AuthorName,AdditionYear,BookMRP,Description,price,DealType,Location,Path,email)value(?,?,?,?,?,?,?,?,?,?,?,?)";
...
//            ps.setString(3,"Location");
 ...
 public List<bookdtl> getBooksByCategory(String category) {
    List<bookdtl> list = new ArrayList<bookdtl>();
    bookdtl b = null;
    try {
        String query = "SELECT * FROM book WHERE BookCategiries = ? AND Location = ? AND BookName LIKE ?";
